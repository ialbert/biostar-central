...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option('--prog', dest='prog', default="pg_dump", help='the postgres program default=%default'),
...
def main(pg_user, hourly, prog, outdir):
    ...
    db_file = "%s-%s-%s.sql.gz" % (pg_name, biostar3.VERSION, tstamp)
    db_file = abspath(outdir, db_file)

    params = dict(
        pg_user=pg_user,
        pg_name=pg_name,
        version=biostar3.VERSION,
        db_file=db_file,
        prog=prog,
    )
