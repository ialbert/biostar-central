...
def announce(request):
    ...
    # Get the torrent for the tracker
    torrent, torrent_created = Torrent.objects.get_or_create(info_hash=info_hash)

    if torrent.disabled:
        return error_response('torrent is disabled in tracker')
    ...
    # Update the stats for torrents
    torrent.uploaded = F('uploaded') + uploaded
    torrent.downloaded = F('downloaded') + downloaded

    # Handle the events
    event = request.GET.get('event', None)

    seed_change = leech_change = 0
    ...
    if event == 'started':
        ...
        torrent.completed = F('completed') + 1

    torrent.seeds = F('seeds') + seed_change
    torrent.leeches = F('leeches') + leech_change
    torrent.save()

    # everything is ok, let's make response
    peers = Peer.objects.filter(torrent=torrent).order_by('?')[:20]

    # find the current stats on torrents
    complete = torrent.peer_set.filter(left=0).count()
    incomplete = torrent.peer_set.exclude(left=0).count()
