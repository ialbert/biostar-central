...
BioStar is a [Python][python] and [Django][django] based Q&A web software modeled after
...
no changes on any operating system that supports Python. The software 
runs the **BioStar Bioinformatics Q&A** site at: http://www.biostars.org
...
have them on your system. Switch to the *libs* directory and unpack the *depot.zip* :
...
You will need to initialize a number of environment variables then run the server:
...
In general you will first need to initialize environment variables 
then start the server. There are a number of example environments in the `conf` folder.
...
the ``SELENIUM_TEST_LOGIN_TOKEN`` variable must be set in your Django settings file. See the `conf/selenium.env` file:
...
* [django_openid_auth][django_openid_auth] and [python_openid][python_openid] for openid authentication
